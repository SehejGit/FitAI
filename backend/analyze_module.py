...
def analyze_dumbbell_press(video_path, output_video_path=None):
    ...
    # Variables to track dumbbell press state
    rep_count = 0
    rep_stage = None  # "down" or "up"
    shoulder_angles = []
    elbow_angles = []
    good_frames = 0
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                right_shoulder_angle = calculate_angle(
                    ...
                    [right_shoulder.x, right_shoulder.y - 1]
                )
                ...
                if rep_stage is None:
                    rep_stage = "down" if avg_elbow_angle < 90 else "up"
                elif rep_stage == "up" and avg_elbow_angle < 90:
                    rep_stage = "down"
                elif rep_stage == "down" and avg_elbow_angle > 160:
                    rep_stage = "up"
                    rep_count += 1

                # Store angles for analysis
                shoulder_angles.append((left_shoulder_angle + right_shoulder_angle) / 2)
                ...
                cv2.putText(frame, f"Stage: {rep_stage}", (10, 70),
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    result = {
        "rep_count": rep_count,
        "form_analysis": {
            "min_shoulder_angle": min(shoulder_angles),
    ...
    if fa["min_elbow_angle"] > 90:
        result["feedback"].append("Lower the weights more, aim for elbow angle <90° at the bottom.")
    if fa["max_elbow_angle"] < 160:
        result["feedback"].append("Extend your arms fully at the top, aim for elbow angle >160°.")
    if not result["feedback"]:
        result["feedback"].append("Excellent form! Keep up the good work.")
...
def analyze_banded_squat(video_path, output_video_path=None):
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                right_hip_angle = calculate_angle(
                    [right_knee.x, right_knee.y],
                    [right_hip.x, right_hip.y],
                    [right_hip.x, right_hip.y - 1]
                )
                left_ankle_angle = calculate_angle(
                    [left_knee.x, left_knee.y],
                    [left_ankle.x, left_ankle.y],
                    [left_ankle.x + 1, left_ankle.y]
                )
                right_ankle_angle = calculate_angle(
                    [right_knee.x, right_knee.y],
                    [right_ankle.x, right_ankle.y],
                    [right_ankle.x + 1, right_ankle.y]
                )

                # Track rep stages
                avg_knee_angle = (left_knee_angle + right_knee_angle) / 2
                if rep_stage is None:
                    rep_stage = "down" if avg_knee_angle < 90 else "up"
                elif rep_stage == "up" and avg_knee_angle < 90:
                    rep_stage = "down"
                elif rep_stage == "down" and avg_knee_angle > 160:
                    rep_stage = "up"
                    rep_count += 1
                ...
                ankle_angles.append((left_ankle_angle + right_ankle_angle) / 2)
                good_frames += 1
    ...
    if fa["min_knee_angle"] > 90:
        result["feedback"].append("Squat deeper, aim for knee angle <90° at the bottom.")
    if fa["max_knee_angle"] < 160:
        result["feedback"].append("Stand up fully at the top, aim for knee angle >160°.")
    if fa["min_hip_angle"] > 70:
        result["feedback"].append("Keep your chest up and maintain proper hip hinge.")
    if not result["feedback"]:
        result["feedback"].append("Excellent form! Keep up the good work.")
...
def analyze_band_pull_apart(video_path, output_video_path=None):
    ...
    writer = None
    if output_video_path:
        writer = setup_video_writer(output_video_path, fps, frame_width, frame_height)
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            elif pull_stage == "apart" and initial_shoulder_distance is not None:
